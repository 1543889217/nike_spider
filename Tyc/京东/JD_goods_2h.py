...
class Spider(object):
        ...
        try:
            ...
            for i in range(0, int(pages_num)):
                ...
                try:
                    ...
                    try:
                        time.sleep(0.2)
                        response = requests.get(url=comment_url, headers=headers, allow_redirects=False, timeout=30)
                    except:
                        time.sleep(0.2)
                        response = requests.get(url=comment_url, headers=headers, proxies=proxies, allow_redirects=False, timeout=30)
                ...
                # 总销量
                # sales = json.loads(comment.group())['productCommentSummary']['commentCount']
    ...
    def run(self, lock):
        for i in range(1000000):
            lock.acquire()
            redis_url_num = self.redis_example.llen('jing_dong_2h')
            if str(redis_url_num) == '0':
                print('**************\nRedis消息队列中url为空，程序等待中.....\n进程等待中......\n*************')
            item = self.redis_example.brpop('jing_dong_2h', timeout=3600)[1]
            lock.release()
