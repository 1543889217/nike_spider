...
class Spider(object):
    ...
    def parse_goods_url(self, data):
        ...
        self.parse_goods_shop(goods_dict)

    # 解析获取商品店铺名
    def parse_goods_shop(self, goods_dict):
        try:
            url = 'https://tui.taobao.com/recommend?seller_id={}&item_ids={}&floorId=42296&pSize=4&callback=detail_pine&appid=6862&count=6&pNum=0'.format(goods_dict['sellerId'], goods_dict['itemId'])
            headers = {
                'Referer': 'https://item.taobao.com/item.htm?spm=a230r.1.14.231.372d4af9WwFN4v&id=615090364236&ns=1&abbucket=6',
            ...
            try:
                ...
                try:
                    ...
                    time.sleep(0.2)
                    response = requests.get(url=url, headers=headers, proxies=proxies, allow_redirects=False, timeout=30)
            json_data = json.loads(response.text.replace('detail_pine(', '').replace(');', ''))

            goods_dict['shop_name'] = json_data['result'][0]['shopName']
            print('--------开始写入数据----------------')
            print(goods_dict)
            items = json.dumps(dict(goods_dict), ensure_ascii=False) + '\n'
            with open('./{}_taobao_url.json'.format('test'), 'ab') as f:
    ...
    #     try:
    #         url = 'https://rate.taobao.com/detailCommon.htm?auctionNumId={}&userNumId={}'.format(goods_dict['itemId'], goods_dict['sellerId'])
    #         headers = {
    ...
    def run(self, lock):
        f = open('./pk_taobao_url.json', 'r', encoding='utf-8')
        goods_data_list = []
