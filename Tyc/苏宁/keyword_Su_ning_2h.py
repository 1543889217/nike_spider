...
class Spider(object):
    ...
    def goods_price(self, goods_dict):
        try:
            ...
            price_url = 'https://pas.suning.com/nspcsale_0_{}_{}_{}_20_021_0210101_315587_1000267_9264_12113_Z001___R9000361_1.39_0___000165956__.html?callback=pcData'.format(partNumber, partNumber, vendorCode)

            try:
                time.sleep(0.2)
                response = requests.get(url=price_url, headers=headers, proxies=proxies, allow_redirects=False, timeout=30)
            except:
                try:
                    time.sleep(0.2)
                    response = requests.get(url=price_url, headers=headers, proxies=proxies, allow_redirects=False, timeout=30)
    ...
    def parse_comment_num(self, goods_dict):
        try:
            ...
            try:
                time.sleep(0.2)
                response = requests.get(url=comment_num_url, headers=headers, proxies=proxies, allow_redirects=False, timeout=30)
            except:
                try:
                    time.sleep(0.2)
                    response = requests.get(url=comment_num_url, headers=headers, proxies=proxies, allow_redirects=False, timeout=30)
                ...
                if len(items) == 0:
                    ...
                    for data in items:
                        ...
                        if self.start_time <= data['publishTime']:
                            ...
                            goods_comment['content_id'] = str(data['commodityReviewId'])
    ...
    def likes_comments(self, goods_comment):
        try:
            ...
            self.hdfsclient.new_write('/user/cspider_daily/nike_2h/ecommerce/{}/{}/53_{}_Suning_nike_{}.json'.format(self.date_time, self.h2_name, time.strftime('%Y%m%d'), self.pid), item, encoding='utf-8')
        except:
