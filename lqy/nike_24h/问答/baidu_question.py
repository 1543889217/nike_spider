...
# file_name = r"./other/baiduzhidao/question-{}.log".format(now_table)
...
def run(url,pn,word):
    ...
    for j in urls:
        ...
        comments_view = ''
        #  详情页
        response = requests.get(j, proxies=proxies,timeout=10, headers=headers1)
        print(j)
        response.encoding = 'gbk'
        ...
        hdfs.new_write('/user/cspider_daily/nike_daily/qa/%s/48_%s_%s_baidu_question.json' % (
...
def ping(qu_id,j,word,que_dates):
    ...
    response = requests.get(j, timeout=10, proxies=proxies,headers=headers)
    response.encoding = 'gbk'
    html = etree.HTML(response.text)
    title = re.findall(r'<title>(.*?)_百度知道</title>', response.text, re.S | re.M)[0]
    ...
    for o in ping_autor:
        ...
        care = {"platform": "百度知道", "date": sj_pingtime[nums], "time": '', "author": o, "title": ''.join(title),
                "content": ccccc,
                "url": j, "is_topics": '否', "floor": '',"topic_date":sj_pingtime[nums],"topic_time":'',
