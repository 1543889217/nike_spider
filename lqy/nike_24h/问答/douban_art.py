...
def main(word,):
    ...
    for j in range(10000):
        ...
        for i in data:
            ...
            # TODO
            hdfs.new_write('/user/cspider_daily/nike_daily/article/%s/174_%s_%s_douban.json' % (
...
def comment(title,response,href,topic_id,date,topic_date,topic_time):
    ...
    for i in text:
        caree = {"platform": "豆瓣评论", "source_date": "".join(date).split(' ')[0],
                 ...
                 "date": dates,"topic_date":topic_date,"topic_time":topic_time,
                 'time': times,
                 "title": "".join(title), "comments_count": '', "author": "".join(name[num]),
                 "content": i, "floor": '', "keyword": '',
                 "content_id": '', "topic_id": topic_id,
                 "source_url": href, "comment_url": href, "views": '', "likes": '',"reposts_count":"",
                 "author_id": '', "dislikes": '',
        ...
        data = json.dumps(caree, ensure_ascii=False) + '\n'
        # TODO
        hdfs.new_write('/user/cspider_daily/nike_daily/articlecomments/%s/181_%s_%s_douban_comment.json' % (
            daily_date, daily_date, sjc), data,
                       encoding='utf-8')
        num += 1
...
if __name__ == '__main__':
    hdfs.makedirs('/user/cspider_daily/nike_daily/article/{}'.format(daily_date)) # 创建每日文件夹
    hdfs.makedirs('/user/cspider_daily/nike_daily/articlecomments/{}'.format(daily_date)) # 创建每日文件夹
