...
def write_json(href,click,replay_no,autor,carseries):
    ...
    if biaoqian == '问答':
        ...
        cars = {"platform": "天涯论坛","carseries": carseries, "author": autor, "date":zhu_j_date,
                ...
                "series_url": '', "topic_count": '', "reply_count": '', "pick_count": '', "topic_categroy": '',
                "topic_type": '',"topic_id":topic_id[:-2],"topic_date":zhu_j_date,"topic_time":zhu_j_time,
                'insert_time': str(datetime.datetime.now()).split('.')[0],
                'update_time': str(datetime.datetime.now()).split('.')[0],
                "post_client": '','favorite':'',"identification":'',"follows":'',"file_code":'43',"reposts_count":"",
        ...
        # TODO
        hdfs.new_write('/user/cspider_daily/nike_daily/forum/%s/43_%s_%s_tianya_forum2.json' % (
    ...
    # 评论
        ...
        for oo in range(len(floor)):
            ...
            p_time = date.split(' ')[0].split('-')
            dateC = datetime.datetime(int(p_time[0]), int(p_time[1]), int(p_time[2]))
            p_time = time.mktime(dateC.timetuple())
            if news_start_time <= p_time <= nows_time:
                ...
                'reply_no': reply_no[oo-1], "is_topics": '否', "views": click,"topic_date":zhu_j_date,"topic_time":zhu_j_time,
                "likes": like, 'signin_time': '', " identification": '', "from": '', "series_url": '', "brand": '',
                "is_elite": is_elite, "topic_count": '', "reply_count": '', "pick_count": '', "topic_categroy": '',
                "topic_type": '',"topic_id":topic_id[:-2],
                'insert_time': str(datetime.datetime.now()).split('.')[0],
                'update_time': str(datetime.datetime.now()).split('.')[0],
                "author_id": autor_id, "content_id": '',"reply_floor":'',"file_code":'43',"reposts_count":"",
                        "audiourl": '', "imageurl": ""
                }
            # cars = {"platform": "天涯论坛", "carseries": carseries, "author": autor, "date": date.split(' ')[0],
            #         'time': date.split(' ')[1], "title": title, "content": content, "url": href, "floor": floor[oo-1],
