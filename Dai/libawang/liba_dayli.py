...
class Spider(object):
    ...
    def __write_news_jsonfile(self, item):
        ...
        self.hdfsclient.new_write('{}/{}/185_{}_{}_liba_news.json'.format(self.file_path, str(datetime.now()).split(' ')[0].replace('-', ''), str(datetime.now()).split(' ')[0].replace('-', '_'), self.time_time), item,encoding='utf-8')
...
if __name__ == "__main__":
    ...
    pool.apply_async(app_run, args=(file_path, ))
    # print(2)
    pool.close()
    # 程序计时，八小时后结束任务
