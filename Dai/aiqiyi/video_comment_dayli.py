...
class AiQiYi(object):
    ...
    def get_video_page(self, url, keyword):
        ...
        if response.status_code == 200:
            ...
            if int(comment_count) > 0:
                self.get_comment(video_id, page, url, video_info['name'], comment_count, keyword, dt.split(' ')[0], dt.split(' ')[1])  # 获取评论
    ...
    def get_comment(self, video_id, page, source_url, title, comment_count, keyword, source_date, source_time):
        """
        ...
        if int(comment_count) > 20*page:  # 判断评论数量，进行翻页操作
            page += 1
            self.get_comment(video_id, page, source_url, title, comment_count, keyword, source_date, source_time)
    ...
    def write_news_jsonfile(self, item):
        ...
        try:
            # str_time = time.time()
            self.hdfsclient.new_write('{}/{}/111_{}_{}_aiqiyi_video.json'.format(self.file_path, str(datetime.now()).split(' ')[0].replace('-', ''), str(datetime.now()).split(' ')[0].replace('-', '_'), self.time_time), item,encoding='utf-8')
    ...
    def write_comment_jsonfile(self, item):
        ...
        #     f.write(item.encode("utf-8"))
        try:
            self.hdfsclient.new_write('{}/{}/112_{}_{}_aiqiyi_video_comment.json'.format(self.comment_apth, str(datetime.now()).split(' ')[0].replace('-', ''), str(datetime.now()).split(' ')[0].replace('-', '_'), self.time_time), item,encoding='utf-8')
...
if __name__ == "__main__":
    ...
    file_path = '/user/cspider_daily/nike_daily/video'
    # comment_path = sys.argv[2]
    comment_path = '/user/cspider_daily/nike_daily/videocomments'
    try:
