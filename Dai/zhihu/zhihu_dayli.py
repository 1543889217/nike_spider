...
class ZhiHuSpider(object):
                    ...
                    try:
                        ...
                        if news_type == 'answer':  # 问答类信息
                            ...
                            if float(start_time) <= float(get_news_time) <= float(end_time):
                                ...
                                # if topic_id not in self.set_list:
                                if not self.redis_example.sismember('nike_dayli_zhihu', topic_id):
                                    self.queue.put(item)
                                    self.redis_example.sadd('nike_dayli_zhihu', topic_id)
    ...
    def get_answers_page(self, url, question_title, source_url, keyword, views, topic_time_all):
        ...
        for news in data_list:
            ...
            if float(start_time) <= float(get_news_time) <= float(end_time):
                news_id = news['id']
                # if news['id'] not in self.set_list:
                if not self.redis_example.sismember('nike_dayli_zhihu', news_id):
                    self.queue.put(item)
                    # self.set_list.append(news['id'])
                    self.redis_example.sadd('nike_dayli_zhihu', news_id)
                # self.write_news_jsonfile(item)
                comment_id = news['id']
    ...
    def get_comment_info(self, url, question_title, source_url, keyword, topic_id, topic_time_all):
        ...
        if str(status_code) == '200':
            ...
            for comments in comment_data:
                ...
                # if comments['id'] not in self.set_list:
                if not self.redis_example.sismember('nike_dayli_zhihu', comment_id):
                    self.queue.put(comment_item)
                    # self.set_list.append(comments['id'])
                    self.redis_example.sadd('nike_dayli_zhihu', comment_id)
            is_end = data['paging']['is_end']
            if not is_end:
                next_url = data['paging']['next']
...
file_path = '/user/cspider_daily/nike_daily/qa'
...
def write_news(queue):
    ...
    while True:
        ...
        if q_size > 0:
            ...
            try:
                hdfsclient.new_write('{}/{}/47_{}_{}_zhihu.json'.format(file_path, str(datetime.now()).split(' ')[0].replace('-', ''), str(datetime.now()).split(' ')[0], time_time).replace('-', '_'), total_item,encoding='utf-8')
            except:
                logging.error(traceback.format_exc())
            zero_num = 0
        else:
