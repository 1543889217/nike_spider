...
def write_news(queue):
    ...
    while True:
        ...
        if q_size > 0:
            ...
            try:
                hdfsclient.new_write('{}/{}/{}/47_{}_{}_zhihu_talk.json'.format(file_path, file_name_time.split(' ')[0].replace('-', ''), hour_name, str(datetime.now()).split(' ')[0], time_time).replace('-', '_'), total_item,encoding='utf-8')
            except:
