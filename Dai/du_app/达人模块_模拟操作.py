...
class DouYin():
    ...
    def __init__(self):
        # 通过系统时间自动计算时间间隔
        # date = datetime.now() - timedelta(days=1)  # 七天前的时间，不包括今天
        # str_time = str(date).split(' ')[0]
        # yesterday = datetime.now() - timedelta(days=1)  # 昨天时间
        # now_time = str(yesterday).split(' ')[0]
        # print('爬取时间段：{}到{}'.format(str_time, now_time))
        ...
        self.device_1 = Android('DWH9X17216G01564')
    ...
    def main_work(self):
        ...
        for i in range(10):
            time.sleep(1.5)
            people_btn = self.poco(name='com.shizhuang.duapp:id/tv_about')
            for people in people_btn:
                get_text = people.get_text()
                if get_text not in self.set_people_list:
                    self.set_people_list.append(get_text)
                    print(get_text)
                    time.sleep(2)
                    people.click()
                    time.sleep(5)
                    self.is_work = True
                    self.question_page()  # 问题列表页操作
                    time.sleep(2)
                    self.device_1.keyevent('KEYCODE_BACK')
                    time.sleep(3)
    ...
    def question_page(self):
        for i in range(100):
            time_bn = self.poco(name='com.shizhuang.duapp:id/tv_time')
            if not time_bn:
                continue
            for btn in time_bn:
                ...
                print(get_time)
                # 做时间判断部分---------------
                if ':' not in get_time and '-' in get_time:
                    get_time = get_time + ' 01:01'
                get_time = get_time.replace('.', '-')
                if ':' not in get_time:
                    get_time = get_time + ' 01:01'
                get_news_time = time.mktime(time.strptime(get_time, "%Y-%m-%d %H:%M"))
                ...
                if '天' in get_time:
                    ...
                    if '1' not in get_time and '2' not in get_time and '3' not in get_time and '4' not in get_time:
                        print('时间不符合')
                        self.is_work = False
                        return
    ...
    def time_change(self, str_time):
        ...
        if '秒' in str_time or '刚刚' in str_time:
            get_time = str(datetime.now()).split('.')[0][:-3]
            return get_time
            ...
            two_days_ago = str(two_days_ago).split('.')[0][:-3]
            return two_days_ago

        elif '年' in str_time:
            str_time = str_time.replace('年', '-').replace('月', '-').replace('日', '')
            return str_time
        elif '月' in str_time and '日' in str_time:
            str_time = str_time.replace('年', '-').replace('月', '-').replace('日', '') + '01:10'
            return str_time
